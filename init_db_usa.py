...
def get_latest_article():
    ...
    section_tags = ["news", "tech", "money"]

    for section_tag in section_tags:
        articles = f'https://www.usatoday.com/{section_tag}/'

    data = requests.get(articles, headers=headers)
    soup = BeautifulSoup(data.text, 'html.parser')

    more_top_stories = soup.select('body > main > div.gnt_pr > div.gnt_m.gnt_m_flm > a')

    for story in more_top_stories:

        if story.get("href"):
            a = story.get("href")

        if a.startswith("/") is True:
            url = a
            if url.startswith("/s") is True:
                ...
                for content in contents:
                    ...
                    hour_later = date_time_obj + datetime.timedelta(hours=14)
                    ...
                    if target_url[:24] == "https://www.usatoday.com" and og_desc is not None:
                        doc = {
                            ...
                            'datetime': hour_later,  # 최신 시간순으로 정렬하기 위한 시간
                            'media': 'USA TODAY',
